...
-- DROP FUNCTION public.pgr_fromatob(character varying, double precision, double precision, double precision, double precision, integer, integer);
...
BEGIN
	...
	dayval := 'd' || day::text || '_t' || daypart::text;

	RAISE NOTICE 'Day val= (%)', dayval;
        ...
        sql := 'SELECT  id, geom,  a_nom_comp,  source, target, 
				ST_Reverse(geom) AS flip_geom FROM ' ||
                        'pgr_dijkstra(''SELECT id, source::int, target::int, '
                                        ||  '(cost +  ' || dayval || ') as cost, (reverse_cost + ' || dayval || ') as reverse_cost FROM '
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION public.pgr_fromatob(character varying, double precision, double precision, double precision, double precision, integer, integer)
  OWNER TO postgres;